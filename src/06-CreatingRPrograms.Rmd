...
By `base`, we simpy mean the functions, datasets, and arguments that come 
...
are doing is to spend at least sometime checking out if there is a `package`
...
5) Add interactivity to your R code (make zoomable maps or plots)
...
Comprehensive R Archive Network or CRAN. In order for you to work with them
...
In the above command call ggplot uses the data frame `dat` and the aesthetics (`aes`)
...
we want to use a point geometry to display this relationship (`geom_point`). 


Note: ggplot connects a series of arguments using `+` operator. This is unique 
to ggplot, but it's a helpful way to make complex plots without 100s of assigned 
plot names. 
