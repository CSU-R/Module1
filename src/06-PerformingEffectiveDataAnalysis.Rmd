...
```{r}
# Load the data
# The "Footnote" column has hyphens,
# which only display correctly if we specify "UTF-8" encoding
...
covid <- covid[!(covid$Age.group %in% c("All ages", "All Ages")),]
```
...
# The COVID.19.Deaths vector doesn't have row names,
# so we specify the bar labels with names.arg 
...
barplot(covid_nm_f$COVID.19.Deaths, names.arg = covid_nm_f$Age.group, 
        ...
        main = "Deaths By Age Group")
```
Now, if we wanted to do this for all states in our dataset, this would take a lot of code.
But, did you notice that the code we wrote in each case was very similar?
...
- `for` tells R that we want to repeat code multiple times. When R "sees" the `for` command, it knows that the code that follows will be repeated.
- the parentheses (`(` and `)`) specify a vector that will be looped over (`vec` in this example), and a variable name to use while looping (`i` in this example). On each iteration of the loop, the variable (`i`) will have a different value. In this example, the __first__ time through the loop, `i` will have the value of the __first__ element of `vec` (`"a"`), the __second__ time through the loop, `i` will have the value of the __second__ element of `vec` (`"b"`), etc. The name _for-loop_ is common in many programming languages, which reflects the fact that R is running the loop _for each_ element of the vector.
...
for(i in c("Colorado", "Wyoming", "New Mexico")){
  ...
  png(file_name)
  par(mar = c(9, 4, 2, 2))
  barplot(covid_state_f$COVID.19.Deaths, names.arg = covid_state_f$Age.group, 
          las = 2, 
...
```{block, type="caution"}
This is some of the longest and most complex code that we've discussed so far! It's important that you fully understand what each line is doing, so take your time and review the code chunk above until you're comfortable with it.
```
...
So far, we've been generating plots for the females only, but we can include another loop which cycles through each Sex for each state (changes to the code are marked with comments):
...
It's possible to have a set of three nested for-loops or even more, but generally this is not wise practice, and in most cases there is a way to accomplish the same goal without so much looping.
```
...
Let's return to the COVID example for motivation.
...
Since R is a programming language, it is not immune to the common debates between programmers regarding proper formatting.
...
But different programmers (and sometimes programming communities) will have different ideas about what is best.
...
Mainly, we raise these differences because you may have to read code written by someone with different conventions from yourself.
...
are doing is to spend at least some time checking out whether there is a `package`
...
Here is an example with an extremely common plotting package `ggplot2` that has been mentioned in the course (and that you have probably already seen on the internet)
...
Our package is now loaded, and we have a manual for reference so let's use it!
...
This can be difficult to get used to, but can be very powerful once you get the hang of it.
```
...
ggplot(dat, aes(x = driver, y = response)) + 
  ...
  labs(title = "Driver vs. Response",
       x = "Driver", y = "Response")   # Add labels to the plot
```
...
ggplot(mtcars, aes(x = hp, y = disp, color = mpg, label = car)) + 
  geom_point() + 
  geom_text(color = "black", hjust = 0,
            nudge_x = 5, alpha = 0.8, size = 2) +   # Annotate each car
  scale_color_viridis_c(option = "D") +             # Change color scale to use
  labs(title = "Motor Trend Car Comparison",
       x = "Horsepower", y = "Displacement", color = "MPG")
```
...
Let's look at some examples of `base` R indexing and compare that to how `dplyr` accomplishes the same task.
...
Basically, the pipe operator says "Take the thing on the left and use it as the first argument for the function on the right".
