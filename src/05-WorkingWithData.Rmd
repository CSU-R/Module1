...
```{block, type="progress"}
Using the clean athletes data, make boxplots of Weight for athletes whose Sport is "Golf"
Give it an appropriate title and axis labels.
Comment on the differences in height between the different categories.
How many athletes have no height reported (that is, how many have `NA` for Height) and how many athletes have a height?
...
#This may break everything for you alex. Unclear. Helps print help files. 
library(printr)
...
monitoring locations. But how do we use this function? Here we can use the `?`
operator to get a better sense of how it works. 
...
To look at variation per year we can use the base function called `tapply` which can take the mean, max or any summary
function of groups of data. In this case we want to look
at the mean river flow for each water year.
