...
```{block, type="bonus"}
In other programming languages, a distinction is made between _different types of numeric data_.
For example, integer, floating point, and double precision numbers.
_R also makes this distinction_, and you _can_ be this specific if you like, but if you don't specify what type of numeric data you have, R will "make the appropriate decisions" for you behind the scenes.
R also has the capacity to handle complex numbers, but we will not describe that in detail here.
```
...
```{r}
grep("a", g)   # This combines multiple strings together into one string!
```
