...
That is, we created a thing called <span style="color:black">`x`</span> , and _assigned_ to it the value of <span style="color:black">`1+1`</span> using the _assignment operator_, <span style="color:black">`<-`</span>.
Formally <span style="color:black">`x`</span> is called an object, but we'll talk about that more objects and assignments later.
...
If you don't remember the exact name of a function, or would like to search for general matches to a topic, then you can use <span style="color:black">`??`<span>.
For example, trying <span style="color:black">`?Print`<span> produces an error, because there is not <span style="color:black">`Print`<span> function (remember, R is case sensitive), so there's no documentation to go with it.
...
```{block, type="bonus"}
Programmers have a sense of humor, too! Try running <span style="color:black">`????print`<span> to see a small joke. Remember, comedic taste varies!
```
...
```
```{block, type="bonus"}
Want to know more about R syntax? Try typing <span style="color:black">`?Syntax`<span> in the R console (then press `Enter`).
```
...
```{block, type="bonus"}
Since backslash also has special meaning itself, if you want a backslash in your string, you need to use another escape character, like so: <span style="color:black">`g <- "here is a backslash: \\"`<span>

To see a list of special characters, try typing <span style="color:black">`?Quotes`<span> into the R console
```
...
However, you _cannot_ create a vector of negative length (e.g. <span style="color:black">`logical(-1)`<span> won't work), or a fractional length (e.g. <span style="color:black">`character(12.7)`<span> won't work).
```
...
```
```{block, type="bonus"}
Other examples of summary functions include <span style="color:black">`max`<span>, <span style="color:black">`min`<span>, <span style="color:black">`mean`<span>, and <span style="color:black">`sd`<span>. We'll talk about these and other summary functions later.
```
...
```
```{block, type = "bonus"}
When you're working with large data sets, it's often helpful to see just the first few results instead of printing the entire thing. You can use <span style="color:black">`head()`<span> to print the first six rows. 
```
...
```{block, type="bonus"}
<span style="color:black">`NA`<span> values have interesting behavior in R.
Generally, anything that "touches" an <span style="color:black">`NA`<span> becomes an <span style="color:black">`NA`<span>.
You can try out these commands for yourself to see what happens:

<span style="color:black">`NA * 0`<span>

<span style="color:black">`NA - NA`<span>

<span style="color:black">`c(NA, 1, 2)`<span>
...
```{block, type="bonus"}
As we've seen, type conversion is sometimes performed automatically, specifically when using the combine function (<span style="color:black">`c`<span>).

To understand more about this, try typing <span style="color:black">`?c`<span> to bring up the documentation, and have a look at the "Details" section.
```
...
### Lists
```{block, type="bonus"}
[purrr](https://purrr.tidyverse.org/) is a very useful library for working with lists. 
```
